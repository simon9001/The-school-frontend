...
### Task 7: Print stylesheet and the printable receipt component
...
- Produces: `<PrintableReceipt payment={…} student={…} schoolName={…} onClose={…} />` — a modal with a Print button. Used by Task 9.

- [ ] **Step 1: Add print rules to the stylesheet**
...
Run: `cd Schoolfrontend && pnpm build`
Expected: build succeeds. Not yet rendered anywhere.

- [ ] **Step 4: Commit**

```bash
cd Schoolfrontend
git add src/index.css src/modules/fees/PrintableReceipt.tsx
git commit -m "Add print stylesheet and printable fee receipt"
```
...
```tsx
            ...
            {issuedReceipt && (
                <PrintableReceipt
                    ...
                    student={student}
                    schoolName="School Financial Management System"
...
**Interfaces:**
- Consumes: `FeeInvoice`, `FeePayment` from `./types`; `Student` from `../students/types`; the print CSS from Task 7.
...
In `FeeCounterPage.tsx`, add the imports:
